...
namespace PredictedAdaptedEncoding
{
    ...
    )noexcept
    {
        ...
        const uint64_t desired_packed_cell = PackedCell64_t::MakeTypedFabricValidPackedCell(
    ...
    /// @param expected_cell 
    /// @return 
    JustifyClaimCas ReadWriteConstructor::TryClaimACellInSlab(PackedCell64_t::AuthoritiveCellView& expected_cell_auth_view, uint64_t* desired_packed_cell) noexcept
    {
        ...
        for (size_t i = 0; i < DEFAULT_MAX_TRIES; i++)
        {
            ...
            const uint64_t desired_cell = PackedCell64_t::SetLocalityInPacked(expected_cell_auth_view.RawCell, LocalityPolicy::CLAIMED);
    ...
    ) noexcept
    {
        ...
        const std::pair<uint64_t, uint64_t> low32_and_probable_high32 = PairedVersionedCellModelOfMode32::GetPairOfLow32FAndHigh32SFromUnsigned64ForFabric(
        ...
        if (!maybe_desired_candidate_occupancy|| *maybe_desired_candidate_occupancy == FABRIC_CELL_SENTINAL)
        {
            return ForceUpdate();
        }
        ...
        if (
            *maybe_desired_candidate_occupancy <= BIT_FAMILY_32_SENTINAL && 
            low32_and_probable_high32.second == FABRIC_CELL_SENTINAL
        )
        {
            uint64_t expected = low32_half_view.RawCell;
            const uint64_t desired = low32_and_probable_high32.first;
            for (size_t i = 0; i < DEFAULT_MAX_TRIES; i++)
            {
        ...
        {
            uint64_t expected_low = low32_half_view.RawCell;
            const uint64_t desired_claimed_low = PackedCell64_t::SetLocalityInPacked(low32_half_view.RawCell, LocalityPolicy::CLAIMED);
            ...
            for (size_t i = 0; i < DEFAULT_MAX_TRIES; i++)
            {
                ...
                {
                    uint64_t expected_high = high32_half_view.RawCell;
